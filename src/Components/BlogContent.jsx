const Img=({url,caption})=>{
    return (
        <div>
            <img src={url} alt="" />
            {caption.level?<p className="w-full text-center my-3 md:mb-12 text-base text-dark-grey" ></p>:""}
        </div>
    )
}
...
const Blogcontent=({block})=>{
   ...
   if(type=="paragraph"){
        return <p dangerouslySetInnerHTML={{__html:data.text}}></p>//avoid the html tag in paragraph  
   }else if(type=="header"){
   ...
   if(type=="list"){
       return <List style={data.style} itmes={data.items}/>
   }
